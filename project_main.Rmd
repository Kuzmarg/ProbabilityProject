...
The p-value in the test of slope coefficient is significantly big. The model here states that our starting assumption is wrong: movies are getting longer. **Nevertheless, the main problem is that real relation is far from linear, so we can not make any conclusions here**.
...
for (category in Categories) {
  ...
  duration_20 = df[strtoi(substring(temp_df$Date, 0, 4)) < 2001,]$Runtime
...
for (year in Years) {
  ...
  cast_of_films = df[strtoi(substring(df$Date, 0, 4)) >= year & 
                   strtoi(substring(df$Date, 0, 4)) < year+1, ]$Cast
